...
# ---- 代理配置（AI 与 YouTube 分开配置）----

class ProxySectionRequest(BaseModel):
    enabled: bool
    url: Optional[str] = None


class ProxyConfigRequest(BaseModel):
    ai: Optional[ProxySectionRequest] = None
    youtube: Optional[ProxySectionRequest] = None


def _proxy_response(mgr):
    cfg = mgr.get_config()
    return {
        **cfg,
        "effective": {
            "ai": mgr.get_ai_proxy_url() or "",
            "youtube": mgr.get_youtube_proxy_url() or "",
        },
    }
...
def get_proxy_config():
    ...
    return R.success(data=_proxy_response(mgr))
...
def update_proxy_config(data: ProxyConfigRequest):
    ...
    mgr = ProxyConfigManager()
    mgr.update_config(
        ai=data.ai.model_dump() if data.ai else None,
        youtube=data.youtube.model_dump() if data.youtube else None,
    )
    return R.success(data=_proxy_response(mgr))
